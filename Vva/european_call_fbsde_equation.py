...
Driver (from Feynman-Kac / Black-Scholes PDE):
    f(t, x, y, z) = -r * y + r * (z / sigma)
...
from scipy.stats import norm  # used only in analytical methods
...
class EuropeanCallEquation(FBSDE):
    ...
    def terminal_condition(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        """
        return torch.clamp(x - self.K, min=0.0)   # x is [Batch, 1]
