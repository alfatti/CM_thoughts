...
def portfolio_clean_value(
    ...
    for cap in portfolio.caplets:
        ...
        if t >= T_pay:
            # Caplet has already paid out — contributes zero to portfolio value.
            continue
        if t > T_reset:
            # Between reset and payment: intrinsic value discounted to t.
            delta = portfolio.tenor.accrual(i)
            P_t_Tpay = disc.PtT(t, T_pay)
            F_i = X_t[:, i - 1]
            intrinsic = delta * P_t_Tpay * np.maximum(F_i - cap.K, 0.0)
            sign = 1.0 if cap.is_long else -1.0
            V += sign * cap.notional * intrinsic
            continue
